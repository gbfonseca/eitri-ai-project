...
export default function SkuSelector(props) {
      ...
      available[variation.name] = variation.values.filter((value) =>
        skus.some(
          (sku) =>
            ...
            sku.available &&
            selectedVariations && Object.entries(selectedVariations)?.every(
              ([varName, varValue]) =>
  ...
  const renderOptionDropdown = (sku, value) => {
    const isAvailable = availableVariationValues[sku].includes(value);
    const isSelected = selectedVariations[sku] === value;
  ...
  return (
    <View color="accent-100" marginTop={marginTop} direction="column" gap={12}>
      {variations?.map((sku, index) => (
        ...
        >
          ...
          <Dropdown
            // placeholder={selectedVariations[sku?.name]}
            contentColor="accent-100"
            value={selectedVariations[sku?.name]}
            onChange={(value) => handleVariationSelect(sku?.name, value)}
            top={0}
          >
            {sku?.values?.map((value) =>
              renderOptionDropdown(sku?.name, value)
            )}
          </Dropdown>
        </View>
      ))}
